  ...
  return (
    <section className="bg-white" style={{ paddingTop: '40px', paddingBottom: '40px' }}>
      <div style={{ padding: '0 5%', marginBottom: '20px' }}>
        <span style={{
          display: 'inline-block', padding: '8px 16px', backgroundColor: '#f59e0b',
      ...
      <div style={{ position: 'relative', minHeight: '400px', overflow: 'hidden' }}>
        <div
          style={{
            transform: isAnimating ? `translateX(${slideDirection * -100}%)` : 'translateX(0)',
            transition: 'transform 0.5s ease-in-out',
          }}
        >
          <div className="flex" style={{ gap: '40px', padding: '0 5%' }}>
            <div style={{ flex: '0 0 35%', paddingTop: '20px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '12px', marginTop: '20px' }}>
          {slides.map((_, index) => (
            <button key={index} onClick={() => { setSlideDirection(index > currentSlide ? 1 : -1); setIsAnimating(true); setProgress(0); setTimeout(() => { setCurrentSlide(index); setIsAnimating(false); }, 300); }}
              style={{ width: currentSlide === index ? '24px' : '8px', height: '8px', borderRadius: '4px', border: 'none',
                backgroundColor: currentSlide === index ? '#f59e0b' : '#d1d5db', cursor: 'pointer', transition: 'all 0.3s ease' }} />
          ))}
        </div>
      </div>
    </section>
